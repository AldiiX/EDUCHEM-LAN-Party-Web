...
public static class Database {
    ...
    public static MySqlConnection? GetConnection() {
        ...
        try {
            conn = new MySqlConnection($"server={(Program.USE_LOCALHOST_CONNECTION ? "localhost" : Program.ENV["DATABASE_IP"])};userid={Program.ENV["DATABASE_NAME"]};password={Program.ENV["DATABASE_PASSWORD"]};database={Program.ENV["DATABASE_NAME"]};pooling=true;Max Pool Size=300;");
            conn.Open();
        } catch (Exception e) {
